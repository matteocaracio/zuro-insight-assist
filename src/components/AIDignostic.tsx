...
const AIRecommendation = () => {
  ...
  const handleGeneratePlan = async () => {
    ...
    setTimeout(() => {
      ...
      setResults({
        ...
        resources: [{
          title: "Guia de Exercícios para Coluna",
          type: "PDF",
          url: "#"
        }, {
          ...
          title: "Nutrição para Recuperação Muscular",
          type: "Artigo",
          url: "#"
        }],
        plan: [{
          day: 1,
          activities: ["Avaliação inicial", "Exercícios básicos de mobilidade", "Aplicação de calor local"]
        }, {
          day: 2,
          activities: ["Descanso ativo", "Alongamentos leves", "Caminhada de 10 minutos"]
        }, {
          day: 3,
          activities: ["Exercícios de fortalecimento", "Mobilização articular", "Técnicas de respiração"]
        }, {
          day: 7,
          activities: ["Reavaliação de progresso", "Ajustes no plano de tratamento", "Incremento na intensidade dos exercícios"]
        }, {
          day: 14,
          activities: ["Avaliação final do ciclo", "Definição de próximos passos", "Orientações para continuidade"]
        }]
      });
      setLoading(false);
    }, 2000);
  ...
  return <div className="space-y-8">
      ...
      <Card className="shadow-md border-gray-200">
        ...
        <CardFooter>
          <Button onClick={handleGeneratePlan} disabled={loading} className="bg-zuro hover:bg-zuro-dark text-white">
            {loading ? <>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {results.resources.map((resource, i) => <Card key={i} className="overflow-hidden">
                        ...
                        <div className="bg-gray-50 px-4 py-3 border-t">
                          <a href={resource.url} className="text-sm text-zuro hover:text-zuro-dark flex items-center" target="_blank" rel="noopener noreferrer">
                            Acessar recurso
              ...
              <Card>
                <CardHeader>
                  <div className="flex justify-between">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => copyToClipboard(results.plan.map(p => `Dia ${p.day}:\n${p.activities.map(a => `- ${a}`).join('\n')}`).join('\n\n'))}>
                      <Copy className="h-4 w-4 mr-1" />
                      Copiar
                    </Button>
                  </div>
                ...
                <CardContent>
                  <div className="space-y-6">
                    {results.plan.map((day, i) => <div key={i} className="relative">
                        <div className="flex items-center mb-2">
                          ...
                          <h4 className="ml-3 font-medium text-gray-900">
                            {day.day === 1 ? "Primeiro Dia" : day.day === 7 ? "Primeira Semana" : day.day === 14 ? "Segunda Semana" : `Dia ${day.day}`}
                          </h4>
                        </div>
                        <div className="ml-4 pl-8 border-l-2 border-dashed border-gray-200">
                          <ul className="space-y-3">
                            {day.activities.map((activity, j) => <li key={j} className="flex items-center">
